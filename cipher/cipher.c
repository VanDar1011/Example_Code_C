...
int main() {
    ...
    unsigned char byte_array[16]; // Khai báo mảng byte có kích thước 16
    int i;

   // printf("Nhập 16 kí tự:\n");

     // Lấy 16 kí tự từ bàn phím và đưa vào mảng byte
     //for (i = 0; i < 16; i++) {
      //  byte_array[i] = getchar(); // Lấy kí tự từ bàn phím
     //}

    // printf("Mảng byte 16 kí tự:\n");
    // for (i = 0; i < 16; i++) {
    //     printf("%02X ", byte_array[i]); // In giá trị byte dưới dạng hex
    // }
    // printf("\n");
    ...
    // Example usage: Encrypt plaintext using AES
  //  AES_Encrypt(byte_array, expandedKey);
    //AES_Encrypt(plaintext, expandedKey);
    AES_Invecrypt(ciphertext, expandedKey);
     //readCirpher(cipher, expandedKey1);
     // Encrypted ciphertext will be in the `plaintext` array
    return 0;
}
