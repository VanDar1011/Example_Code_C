...
void AES_Encrypt(unsigned char* vecto, unsigned char* state, unsigned char* roundKeys) {
    xor (state, vecto);
...
void AES_Invecrypt(unsigned char* vecto, unsigned char* state1, unsigned char* roundKeys1) {
...
void getInputKey(unsigned char key[16]) {
    ...
    while (1) {
        ...
        if (inputLength != 16) {
            printf("Number of character is not 16, please enter 16 character to execute code\n");
        }
        else {
            break; // Thoát khỏi vòng lặp nếu độ dài chuỗi đúng
        }
    }
    for (int i = 0; i < 16; i++) {
    ...
    do {
        ...
        case 1:
            ...
            while ((bytesRead = fread(buffer, 1, sizeof(buffer), inputFile)) > 0) {
                ...
                if (flag == 1) {
                    AES_Encrypt(copyvecto, buffer, expandedKey);
        ...
        case 2:
            ...
            while (fscanf_s(inputFileCir, "%02hhx%02hhx", &byte1, &byte2) == 2) {
                ...
                if (bufferIndex == 16) {
                    if (flag == 1) {
                        equal(buffer, copyvecto);
                        AES_Invecrypt(vecto, buffer, expandedKey);
                        flag++;
                    }
                    else {
                        equal(buffer, copyvecto1);// hai gia tri dang khac nhau 
                        AES_Invecrypt(copyvecto, buffer, expandedKey);
